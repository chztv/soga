...
uninstall() {
    ...
    systemctl stop sogav2
    systemctl disable sogav2
    rm /etc/systemd/system/sogav2.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/sogav2/ -rf
    rm /usr/local/sogav2/ -rf
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        systemctl reset-failed sogav2
        systemctl start sogav2
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
...
show_log() {
    journalctl -u sogav2.service -e --no-pager -f
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
check_status() {
    if [[ ! -f /etc/systemd/system/sogav2.service ]]; then
        return 2
    fi
    temp=$(systemctl status sogav2 | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled sogav2)
...
show_usage() {
    echo "sogav2 管理脚本使用方法: "
    echo "------------------------------------------"
    echo "sogav2                    - 显示管理菜单 (功能更多)"
    echo "sogav2 start              - 启动 soga"
    echo "sogav2 stop               - 停止 soga"
    echo "sogav2 restart            - 重启 soga"
    echo "sogav2 enable             - 设置 soga 开机自启"
    echo "sogav2 disable            - 取消 soga 开机自启"
    echo "sogav2 log                - 查看 soga 日志"
    echo "sogav2 update             - 更新 soga 最新版"
    echo "sogav2 update x.x.x       - 安装 soga 指定版本"
    echo "sogav2 config             - 显示配置文件内容"
    echo "sogav2 config xx=xx yy=yy - 自动设置配置文件"
    echo "sogav2 install            - 安装 soga"
    echo "sogav2 uninstall          - 卸载 soga"
    echo "sogav2 version            - 查看 soga 版本"
    echo "------------------------------------------"
}
