...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        ...
        echo -e "检测到 soga 最新版本：2.8.5，开始安装"
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/vaxilu/soga/releases/download/2.8.5/soga-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/vaxilu/soga/releases/download/2.8.5/soga-linux-${arch}.tar.gz"
    ...
    echo -e "${green}soga v2.8.5${plain} 安装完成，已设置开机自启"
